    ...
    return {
        ...
        onCreate(editor) {
            editor.on('update', ({ state }) => {
                const plainTextContent = state.doc.textBetween(0, state.doc.content.size, '\n\n');
...
const MenuBar = ({editor, summarizeContent,translateContent,loadingSummarize, loadingTranslate}) => {
    ...
    const handleToggleBold = () => {
        ...
        setMenuBarState((prevState) => ({ ...prevState, isBoldActive: !prevState.isBoldActive }));
    };

    const handleToggleItalic = () => {
        editor.chain().focus().toggleItalic().run();
        setMenuBarState((prevState) => ({ ...prevState, isItalicActive: !prevState.isItalicActive }));
    };
    ...
    return (
        <div className="menuBar">
            ...
            <div>
            <SummarizeButton editor={editor} summarizeContent={summarizeContent} loading={loadingSummarize} />
        </div>
            <div>
                <TranslateButton editor={editor} translateContent={translateContent} loading={loadingTranslate} />
...
export const Tiptap = ({setDescription, summarizeContent,translateContent}) => {
    ...
    return (
        <div className="textEditor">
            <MenuBar editor={editor} summarizeContent={summarizeContent} translateContent={translateContent} />
...
const TipTapWithLogger = () => {
    ...
    const summarizeContent = async (editor) => {
        ...
        const ftext = stripHtmlTags(editor.getHTML({from: editor.state.selection.from, to: editor.state.selection.to}))
    ...
    return (
        <div className="App">
            <Tiptap setDescription={setDescription} summarizeContent={summarizeContent} translateContent={translateContent} />
        </div>
    );
};
